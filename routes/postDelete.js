      ...
      if (error) throw new Error(error);
        ...
        if (x.length >= 1) {
          if (type === 'collectionMovie') {
            return res.redirect(`/v1/edit/${sid}/?season=${seasonNumber}&title=${title}&type=collection`);
          }
          else {
            return res.redirect(`/v1/edit/${sid}/?season=${seasonNumber}&title=${title}&type=${type}`);
          }
        }
