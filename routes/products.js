...
router.get("/home", (req, res, next) => {
	...
	let opt1 = selectFunction(
		"select season_id, season_type from videos"
	);
    ...
    if (error) throw new Error(error);
			...
			if (x.length >= 1) {
				...
				for (const item of x) {
				  const { season_id, season_type } = item;
				  const uniqueKey = `${season_id}-${season_type}`;
				...
				try {
					...
					Promise.all(uniqueSeasonData.map(i => {
					    ...
					    return request(url, options).then(data => JSON.parse(data));
					})).then(results => {
